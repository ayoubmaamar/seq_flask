...
mqtt_client_name = "Keyboard-Display"
mqtt_hostname = "oasishunter254.cloud.shiftr.io"
mqtt_port = 1883
mqtt_user = "oasishunter254"
b_shared_key = ''
...
def gen_data(option):
    if option == 0:
        temp = f"{random.randint(-30, 40)}°C"
        print(f"Temperature: {temp}")
        return bytes(str(temp), 'utf-8')
    elif option == 1:
        pressure = f"{random.randint(90000, 110000)}ps"
        print(f"Pressure: {pressure}")
        return bytes(str(pressure), 'utf-8')
    elif option == 2:
        weather = random.choice(['sunny', 'rainy', 'cloudy', 'windy', 'storm'])
        print(f"Weather: {weather}")
        return bytes(weather, 'utf-8')

option = int(input('Enter your option: \n0 - temp, 1 - pressure, 2 - weather: '))
...
while 1:
    ...
    if b_shared_key != '':
        ...
        key = b_shared_key[:16]
        # print(key)
        returnData = gen_data(option)
        timestamp = bytes(str(datetime.datetime.now()), "utf-8")
        iv, cipthertext, tag = encrypt(returnData, key)
        byte_data = iv + BYTE_SEPARATOR + tag + BYTE_SEPARATOR + cipthertext + BYTE_SEPARATOR + bytes(mqtt_client_name,
